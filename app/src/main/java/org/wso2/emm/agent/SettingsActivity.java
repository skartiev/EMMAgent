...
import org.wso2.emm.agent.automate.AgentAutomator;
...
public class SettingsActivity extends Activity implements APIResultCallBack,TokenCallBack {
	...
	// TODO: It's needed to output to configuration file
	private static int tenant = ConfigProperties.TENANT_VALUE;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// Set registration server address to default value & store it in preferences 
		/*if(ipSaved != null && ipSaved==""){	
			ipSaved = ConfigProperties.serverProperty(tenant);
			if(!ipSaved.equals(""))
				mainPref.edit().putString(getResources().getString(R.string.shared_pref_ip), ipSaved.toString());
				mainPref.edit().commit();
		}*/
		...
		// source place of block 16021554

		optionBtn = (Button) findViewById(R.id.button1);	
		
		if(!accessFlag){
			// original version for block unsecured devices
			/*optionBtn.setVisibility(View.GONE);
			ip.setVisibility(View.GONE);*/
			// FIXME: We provide access but be warned
		...
		// block is 16-02-1554
		// start authorization activity after alert dialog
		if(ipSaved != null && ipSaved != ""){
			ip.setText(ipSaved);
			CommonUtilities.setSERVER_URL(ipSaved);
			...
			if(ConfigProperties.AUTO_MODE){
				CommonUtilities.SERVER_IP = ConfigProperties.serverProperty(ConfigProperties.TENANT_VALUE);
			}			
